...
### Changed
- Automatically sync `mobile_app` notifiers from each user's Home Assistant person-linked devices.
- Preserve manual notifiers separately and expose merged notifier lists in onboard manager state and sensors.

## [1.1.2] - 2026-03-27

### Changed
- Align GitHub Actions with the signalk-ha release process by splitting validation into dedicated `validate` and `hassfest` workflows.
- Run lint and tests on version tags (`v*`) so release tags execute the same CI checks.

## [1.1.1] - 2026-03-27

### Changed
- Updated integration branding assets to use the new captain hat image for both icon and logo.
